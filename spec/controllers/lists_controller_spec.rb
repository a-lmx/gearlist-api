...
RSpec.describe Api::V1::ListsController, type: :controller do
  describe "GET #index" do
    context "no user_id" do
      before :each do
        user = User.create(username: 'a', uid: '123', provider: 'google', token: '5678')
        list1 = create :list, name: "JMT", user_id: user.id
        list2 = create :list, name: "PCT", user_id: user.id
        # auth_header = { "Authorization" => "Token token=\"#{user.token}\"" }
        # headers = { "Authorization" => "Token token=\"#{user.token}\"", "Accept" => "application/json" }

        request.env["Authorization"] = ActionController::HttpAuthentication::Token.encode_credentials(user.token)
        get :index
        binding.pry
